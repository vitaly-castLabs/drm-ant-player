import { c as _asyncToGenerator, _ as _defineProperty, M as MediaManager } from './media_manager-dd274ced.js';
    ...
    return _asyncToGenerator(function* () {
      return _this.getHttpEndpoint(data.http_url, {
        ...
        if (response.status >= 200 && response.status < 400) {
          ...
          Logger$1.warn('Backend does not return ok. Retrying in 3 seconds...');
          _this.tryAgainAfterDelay(callbackConnected);
        }
      }).catch(e => {
        Logger$1.warn('Backend is not ready yet. Retrying in 3 seconds...');
        Logger$1.error(e);
        _this.tryAgainAfterDelay(callbackConnected);
      });
    })();
    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this2.httpEndpointUrl != null) {
        ...
        return _this2.getHttpEndpoint(_this2.httpEndpointUrl, {
          ...
          if (response.ok) {
            Logger$1.info("http endpoint returns ok");
            return response.json(); // Parse JSON from body
          }

          throw new Error('Network response was not ok');
        }).then(data => {
          Logger$1.info("Response body -> fqdn :" + data.fqdn + "	websocket_url: " + data.websocket_url + " http_url: " + data.http_url);
          return _this2.checkBackendInstanceUp(data, callbackConnected);
        }).catch(e => {
          _this2.tryAgainAfterDelay(callbackConnected);
          Logger$1.warn('HttpEndpoint is not ready yet', e);
          Logger$1.error(e);
        });
      } else {
    ...
    */
    ...
    this.wsConn.onmessage = event => {
      ...
      if (obj.command == "start") {
        ...
        if (this.debug) {
          Logger$1.debug("received ice server config");
        }
        if (obj.stunServerUri) {
          ...
          if (this.debug) {
            Logger$1.debug("ice servers updated: " + JSON.stringify(this.webrtcadaptor.peerconnection_config.iceServers));
          }
        }
      }
    };
...
export { WebRTCAdaptor as W, WebSocketAdaptor as a };
